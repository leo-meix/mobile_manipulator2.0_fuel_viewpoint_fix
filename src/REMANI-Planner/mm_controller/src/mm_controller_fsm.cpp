...
MMControllerFSM::MMControllerFSM(const ros::NodeHandle& nh) : nh_(nh){
	...
	nh.param("manual_mapping_mode", manual_mapping_mode_, false);
...
void MMControllerFSM::publish_ctrl_cmd(const ros::Time &stamp){//位置速度混装
	...
	mani_cmd_pub_.publish(joint_cmd); // Commented out to stop publishing joint_cmd messages
}
