...
namespace remani_planner{
  ...
  {//双向RRT规划器函数（多起点/多终点规划）
    ...
    getTraj(path_full, yaw_list_full, t_list_full);//获取完整RRT树轨迹

    path.clear();
    t_list.clear();
    yaw_list.clear();

    Eigen::VectorXd temp(traj_dim_);//添加第一个路径点
    temp = path_full[0];
    path.push_back(temp);
    ...
    for(size_t i = 0; i < t_list_full.size(); i++){
      t_total += t_list_full[i];//累加时间间隔
      temp = path_full[i + 1];//获取下一个路径点
      path.push_back(temp);
  ...
  bool RrtPlanning::search(const std::vector<Eigen::VectorXd>& start_pt_list, const std::vector<double>& start_yaw_list, 
    ...
    for(int i = 0; i < start_num; ++i){
      ...
      start_node->node_state = PathNodeRRT::NODE_STATE::IN_TREE;//标记该节点属于正向树
      start_node->g_score = start_g_score_list[i];//从起点到该节点的代价
      start_node->index = calculateValue(start_node);
      start_node->layer = start_layer_list[i];//层级信息
      start_node->singul = start_singul_list[i];//奇异状态
    ...
    for(int i = 0; i < end_num; ++i){
      ...
      end_node->node_state = PathNodeRRT::NODE_STATE::IN_ANTI_TREE;//标记该节点属于反向树
    ...
    while(true){
      ...
      if(tree_count_ > anti_tree_count_){//节点少的树优先扩展
        dir = false;//扩展反树
      }
      else{
        dir = true;//扩展正树
      ...
      // }
      // else{
      sample(rand_s, rand_yaw);//在状态空间随机采样
      // }

      // std::cout << "q_rand: " << rand_s.transpose() << " " << rand_yaw << std::endl;
      q_near = near(rand_s, rand_yaw, dir);//在当前树中寻找距离采样点最近的节点
      ...
      q_new = steer(q_near, rand_s, rand_yaw, time_resolution_);//从q_near向rand_s方向扩展一个步长time_resolution_
      if(q_new == nullptr){
      ...
      if((dir && q_new->node_state == PathNodeRRT::IN_ANTI_TREE) || (!dir && q_new->node_state == PathNodeRRT::IN_TREE)){
        ...
        if(q_near->g_score + q_new->g_score + estimateHeuristic(q_near, q_new) < c_max_){//检查是否为当前最优路径（代价最小）
          // std::cout << "11111111" << std::endl;
          c_max_ = q_near->g_score + q_new->g_score + estimateHeuristic(q_near, q_new);//路径代价计算
      ...
      if(q_new->node_state == PathNodeRRT::EXPAND || (q_new->node_state == q_near->node_state && q_new->g_score > q_near->g_score + estimateHeuristic(q_near, q_new))){
        ...
        if(q_new->node_state == PathNodeRRT::IN_TREE)
          ...
          ++anti_tree_count_;
        rewire(q_new, 0.45);//rewire优化

        q_near_1 = near(q_new->state, q_new->yaw, !dir);//从另一棵树找到q_new最近的节点
        if(q_near_1 == nullptr){
        ...
        q_new_1 = steer(q_near_1, q_new->state, q_new->yaw, time_resolution_);//从另一棵树向q_new扩展
        if(q_new_1 == nullptr){
          // std::cout << "test 4" << std::endl;
        ...
        if(q_new_1->node_state == PathNodeRRT::EXPAND || (q_new_1->node_state == q_near_1->node_state && q_new_1->g_score > q_near_1->g_score + estimateHeuristic(q_near_1, q_new_1))){
          ...
          while( !((q_new->state - q_new_1->state).norm() < 1.0e-2 && fabs(q_new->yaw - q_new_1->yaw) < 1.0e-2) ){
            //当两棵树的节点足够接近时，不断向对方节点扩展，尝试直接连接。
            q_new_2 = steer(q_new_1, q_new->state, q_new->yaw, time_resolution_);//不断从q_new_1向q_new扩展，直到两节点足够接近
    ...
    for(auto it = node_pool_.begin(); it != node_pool_.end(); ++it){//统计两棵树中奇异状态为1或-1的叶子节点数量
    ...
    if(!have_path_){
      ...
      // std::cout << "debug 1\n";
      mergeTree(path_node_1, path_node_2);//合并两棵树
      // std::cout << "debug 2\n";
      return true;
    }
  }
