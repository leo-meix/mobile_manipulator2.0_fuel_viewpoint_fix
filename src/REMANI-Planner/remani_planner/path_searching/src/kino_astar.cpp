...
int KinoAstar::KinoAstarSearchAndGetSimplePath(const Eigen::VectorXd &start_pos, const Eigen::VectorXd &start_vel, const double start_yaw, const int start_singul, const bool start_gripper,
  ...
  start_state.head(2) = start_pos.head(2);//取前两维x、y为底盘平面位置
  start_state(2) = start_yaw;//朝向
  start_state(3) = start_vel.head(2).norm() * start_singul;//模长*标量速度（前进或后退）
  start_state.tail(manipulator_dof_) = start_pos.tail(manipulator_dof_);

  end_state.resize(4 + manipulator_dof_);//终点。这里 kino-A* 搜索的是简化状态（车+关节），不是完整 PVAJ，那是 MINCO 的事。
  end_state.head(2) = end_pos.head(2);
  end_state(2) = end_yaw;
  end_state(3) = 0.0;
  end_state.tail(manipulator_dof_) = end_pos.tail(manipulator_dof_);

  mm_config_->visMM(local_start_goal_pub_, "start", 0, 0.8, start_state.head(3), start_state.tail(manipulator_dof_), start_gripper);//可视化（RViz里画出机械臂）
  ...
  bool start_goal_is_close = (start_pos - end_pos).head(2).norm() < 8e-2 && (start_pos - end_pos).tail(manipulator_dof_).norm() > 1e-1;//底盘位置已经距目标点很近了，但机械臂还差很多时为true，跳过 A*，靠后面的直线插值+机械臂采样解决
  if(continous_failures_count < try_astar_times_ && (!start_goal_is_close) /*&& false*/){//条件满足使用A*去搜索底盘路径。
    // ROS_WARN("ASTAR!");
    search(start_state, end_state, init_ctrl);
  }else{//如果连续失败太多，或底盘与目标太近则不再用A*
  ...
  if(has_path_){//为true，A*成功：从kino轨迹中采样底盘轨迹
    getKinoNode();
    double basetime = 0.0;
