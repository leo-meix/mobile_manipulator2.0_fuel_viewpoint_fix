...
public:
  ...
  inline bool isInflatedOccupied(const Eigen::Vector3d &pt);
  ...
  void getUpdatedBoxIndex(Eigen::Vector3i &min_id, Eigen::Vector3i &max_id, bool reset);
...
private:
  ...
  void ensureProjectedPointCapacity(size_t required_size);
  ...
  ros::Publisher sdf_map_all_pub_, sdf_map_local_inflate_pub_;
...
{
  return getOccupancy(pt) == OCCUPIED;
}

inline bool GridMap::isInflatedOccupied(const Eigen::Vector3d &pt)
{
  if(!isInMap(pt)){
    return true;
  }
  return getInflateOccupancy(pt) == 1;
}
...
inline int GridMap::getOccupancy(Eigen::Vector3i id)
{
  ...
  const double occ = md_.occupancy_buffer_[toAddress(id)];
