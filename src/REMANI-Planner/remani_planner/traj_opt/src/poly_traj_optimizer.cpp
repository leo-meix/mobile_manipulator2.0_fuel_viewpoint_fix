...
namespace remani_planner
{
  ...
  /* main planning API *///使用l-bfgs
  bool PolyTrajOptimizer::OptimizeTrajectory_lbfgs(const std::vector<Eigen::MatrixXd> &iniState_container, //起始状态容器
                                                    const std::vector<Eigen::MatrixXd> &finState_container,//终止状态容器
                                                    const std::vector<Eigen::MatrixXd> &initInnerPts_container, //初始内部点容器
                                                    const std::vector<Eigen::VectorXd> &initT_container, //初始时间分配容器
                                                    const std::vector<int> &singul_container,//奇异性状态容器
                                                    std::vector<Eigen::MatrixXd> &optCps_container, //输出：优化控制点
                                                    std::vector<Eigen::MatrixXd> &optWps_container, //输出：优化路径点
                                                    std::vector<Eigen::VectorXd> &optT_container,//输出：优化时间分配
                                                    std::vector<Eigen::VectorXd> &optEECps_container){//输出：末端控制点
    traj_num_ = initInnerPts_container.size();//轨迹段数
    iniState_container_ = iniState_container;//保存起始状态
    finState_container_ = finState_container;//保存终止状态
    singul_container_ = singul_container;//保存奇异状态
    variable_num_ = 0;//初始化变量数
    SnapOpt_container_.clear();//清空优化器容器
    piece_num_container_.clear();//清空段数容器
    SnapOpt_container_.resize(traj_num_);//重置优化器容器大小
    piece_num_container_.resize(traj_num_);//重置段数容器大小

    if (traj_num_ != (int)initT_container.size()){//判断数据一致性
    ...
    for(int i = 0; i < traj_num_; i++){
      //check
      if(initInnerPts_container[i].cols()==0){//判断内部点数量
        ROS_ERROR("There is only a piece?");
        return false;
      }
      piece_num = initInnerPts_container[i].cols() + 1;//计算段数（内部点+1）
      piece_num_container_[i] = piece_num;//保存每段轨迹的段数
      piece_num_all += piece_num;//累计总段数
      ...
      SnapOpt_container_[i].reset(piece_num);//重置优化器
      variable_num_ += traj_dim_ * (piece_num - 1); //内部点变量
      variable_num_ += piece_num; //时间分配变量数
    }
    
    variable_num_ += traj_dim_ * (traj_num_ - 1); // gear pos 齿轮位置变量（轨迹连接点）
    variable_num_ += 1 * (traj_num_ - 1); // gear angle 齿轮角度变量（轨迹连接角度）

    Eigen::VectorXd x;//优化变量向量
    x.resize(variable_num_);//分配内存
    int offset = 0;
    // inner pt内部点变量
    for(int i = 0; i < traj_num_; i++){
    ...
    RealT2VirtualT(initT, Vt);//实际时间--->虚拟时间
    // gear pos齿轮位置变量
    ...
    lbfgs_params.g_epsilon = 0.0;//梯度收敛阀值
    lbfgs_params.delta = 1e-3;//收敛阀值
    lbfgs_params.max_iterations = 1000;//最大迭代次数

    double final_cost;//最终代价
    
    iter_num_ = 0;//迭代次数优化
    force_stop_type_ = DONT_STOP;//强制停止类型初始化
    ...
    bool good_traj = true;
    good_traj = IsTrajSafe(singul_traj_data);//检查轨迹安全性
    //处理无效函数值错误
    ...
    for(int i = 0; i < traj_num_; i++){
      ...
      optT_container.push_back(SnapOpt_container_[i].get_T1());//时间分配
    }

    return good_traj;//返回轨迹是否安全
  }
  ...
  int PolyTrajOptimizer::astarWithMinTraj(const Eigen::MatrixXd &iniState,
    ...
    /* astar search and get the simple path*///调用A*搜索获取简单路径
    ...
    for(unsigned int i = 0; i < simple_path_container.size(); ++i){//
      ...
      if (piece_num > 1){
        ...
        for (int j = 0; j < piece_num - 1; ++j){
          innerPts.col(j) = (simple_path_container[i])[j + 1]; //使用路径点作为内部约束
        }
      }
      else{
        //路径太短插入中间点
      ...
      for (int j = 0; j < piece_num; ++j){
        angle1 = (simple_path_container[i])[j].tail(manipulator_dof_);//提取机械臂关节角度
        ...
        for(int k = 0; k < manipulator_dof_; ++k){
          if(angle1(k) - angle2(k) >= 0)
            err = angle1(k) - angle2(k);//计算关节角度变化量
          else
          ...
          if(j == 0 || j == piece_num - 1){
            //起始段或结束段：考虑加减速
            if(err <= dist_acc){
              t_acc_vel = sqrt(err / max_joint_acc_);//三角形速度曲线
            }else{
              t_acc_vel = (err - dist_acc) / max_joint_vel_ + 2 * t_acc;//梯形速度曲线
            }
          }else{
            //中间段匀速运动
            t_acc_vel = err / max_joint_vel_;
          }
          max_t = max(t_acc_vel, max_t);//取最慢关节所需的时间
        }
        t_list[j] = max_t;
      }
      t_list[0] *= 1.5;
